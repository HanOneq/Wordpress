...
, default is 0.75, stand for 75% of the browser.", "vc_parallax_cq")
      ...
      function cq_vc_parallax_func($atts, $content=null, $tag) {
          ...
          $output = '';
          $output .= '<div class="cq-parallaxcontainer '.$extra_class.'"  data-speed="'.$speed.'" data-coverratio="'.$coverratio.'" data-holderminheight="'.$holderminheight.'" data-holdermaxheight="'.$holdermaxheight.'" data-extraheight="'.$extraheight.'">';
          $i = -1;
          $imagewidth = null;
          foreach ($imagesarr as $key => $image) {
              ...
              if(wp_get_attachment_image_src(trim($image), 'full')){
                  ...
                  if($textfirst=="on"){
                      ...
                      $output .= '<div class="cq-parallaximage" data-image="'.$return_img_arr[0].'" data-width="'.$return_img_arr[1].'" data-height="'.$return_img_arr[2].'" data-image-mobile="'.$thumbnail.'" data-link="'.$customlinkarr[$i].'" data-target="'.$custom_links_target.'"></div>';
                  }else{
                      $output .= '<div class="cq-parallaximage" data-image="'.$return_img_arr[0].'" data-width="'.$return_img_arr[1].'" data-height="'.$return_img_arr[2].'" data-image-mobile="'.$thumbnail.'" data-link="'.$customlinkarr[$i].'" data-target="'.$custom_links_target.'"></div>';
                      if($contentarr[$i+1]!=""){
                          $output .= '<section class="cq-parallaxsection" style="color:'.$textcolor.';background:'.$textbackground.';padding:'.$padding.';">';
